...
router.put('/:idPicture/:idUser', verifyToken, picturesUserValidationRules, validateInputs, async (req, res) => {
  try {
    const pictureId = req.params.idPicture;
...
router.delete('/:idpicture/:idUser', verifyToken, async (req, res) => {
  try {
    const pictureId = req.params.idpicture;
    const userId = req.params.idUser;
